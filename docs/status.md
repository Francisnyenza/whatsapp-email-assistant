...
Everything below has tests that run and pass. **1 823 tests** (1 452 unit + 371 integration
...
| `@wea/whatsapp` | 215              | Session window, delivery policy, webhook parsing, builders, templates, Cloud API client, command parser                                              |
...
| `apps/worker`   | 438 + 310 (int.) | Ingest, analysis, embeddings, search, summarise, translate, read aloud, ask, draft, deadlines, notify, planner, actions, sweeps, health, metrics     |
| `apps/web`      | 38               | API client (token handling, refresh), Content-Security-Policy, sign-in, mailboxes, phone, settings                                                   |

```bash
pnpm -r test          # 1 452 unit tests
...
| **Subject editing**                | 🔨         | A compose chooses its own: `email alice@acme.com about Q3 saying …`. What cannot be edited is the subject of a _reply_ or a forward, which stay derived (`Re: `, `Fwd: `) — changing one mid-thread is what mail clients do when a conversation drifts, and nothing here can say it.                                                                                                                                                                                                                                                                                                                                                                           |
