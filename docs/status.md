...
Everything below has tests that run and pass. **1 574 tests** (1 283 unit + 291 integration
...
| `@wea/whatsapp` | 169              | Session window, delivery policy, webhook parsing, builders, templates, Cloud API client, command parser                                          |
...
| `apps/worker`   | 323 + 231 (int.) | Ingest, analysis, embeddings, search, summarise, translate, read aloud, ask, draft, deadlines, notify, planner, actions, sweeps, health, metrics |
| `apps/web`      | 38               | API client (token handling, refresh), Content-Security-Policy, sign-in, mailboxes, phone, settings                                               |

```bash
pnpm -r test          # 1 283 unit tests
...
2. **Bcc.** `mime-builder.ts` emits it and nothing reaches it. Cc and reply-all are done;
...
4. **The cluster itself.** The Terraform module provisions the data layer, the KMS key and
...
5. **More metrics than queue depth.** The worker exports `wea_queue_depth`, which covers
   ...
   are invisible.
6. **E2E, load and security suites (Phase 10).** The integration tests reach a real database
