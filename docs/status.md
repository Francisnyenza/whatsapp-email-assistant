...
Everything below has tests that run and pass. **1 798 tests** (1 432 unit + 366 integration
...
| `@wea/whatsapp` | 210              | Session window, delivery policy, webhook parsing, builders, templates, Cloud API client, command parser                                              |
...
| `apps/worker`   | 428 + 305 (int.) | Ingest, analysis, embeddings, search, summarise, translate, read aloud, ask, draft, deadlines, notify, planner, actions, sweeps, health, metrics     |
| `apps/web`      | 38               | API client (token handling, refresh), Content-Security-Policy, sign-in, mailboxes, phone, settings                                                   |

```bash
pnpm -r test          # 1 432 unit tests
